    ...
    if ($isNewer) {
        ...
        try {
            ...
            if ($downloadUrl) {
                ...
                Write-DeployLog -Message "Keine passende Download-URL für Online-Version $onlineVersion auf der Hilfeseite gefunden. Prüfe direkte CDN-Muster..." -Level 'WARNING'

                # Fallback: direct CDN naming patterns are sometimes available before help page metadata is updated.
                $candidateUrls = @(
                    "https://cdn.prusa3d.com/downloads/drivers/PrusaSlicer_Win_standalone_${onlineVersion}.exe",
                    "https://cdn.prusa3d.com/downloads/drivers/PrusaSlicer_Win_standalone_$($onlineVersion -replace '\\.', '_').exe",
                    "https://cdn.prusa3d.com/downloads/drivers/PrusaSlicer_Win_${onlineVersion}.exe",
                    "https://cdn.prusa3d.com/downloads/drivers/PrusaSlicer_Win_$($onlineVersion -replace '\\.', '_').exe"
                )
                ...
                foreach ($candidateUrl in $candidateUrls) {
                    try {
                        ...
                        $bestVersion  = $onlineVersion
                        Write-DeployLog -Message "Fallback-Download-URL gefunden: $downloadUrl" -Level 'INFO'
                        break
                    } catch {
                        Write-DeployLog -Message "Fallback-URL nicht verfügbar: $candidateUrl" -Level 'DEBUG'
                    }
                }
