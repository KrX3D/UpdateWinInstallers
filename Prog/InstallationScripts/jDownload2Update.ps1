...
if ($webContent) {
    ...
    if ($dateMatch.Success) {
        ...
        if ($parseSucceeded) {
            Write-Host ""
            Write_LogEntry -Message "Lokale Timestamp-Version: $($localLastWriteTime)" -Level "INFO"
            Write_LogEntry -Message "Online Timestamp-Version: $($onlineLastWriteTime)" -Level "INFO"
...
if ($FoundFile) {
    ...
    Write_LogEntry -Message "Lokale Timestamp-Version nach Update-Check: $($localLastWriteTime)" -Level "INFO"
} else {
