...
function Compare-VersionComponents {
    ...
    for ($i = 0; $i -lt $n; $i++) {
        $l = if ($i -lt $lp.Count) { [int]$lp[$i] } else { 0 }
        $r = if ($i -lt $rp.Count) { [int]$rp[$i] } else { 0 }
...
if ($localExtPack) {
    $epVer = $localExtPack.Name -replace 'Oracle_VirtualBox_Extension_Pack-([\d.]+-[\d]+)\.vbox-extpack','$1'
...
if ($localExtPack2) {
    $ev = $localExtPack2.Name -replace 'Oracle_VirtualBox_Extension_Pack-([\d.]+-[\d]+)\.vbox-extpack','$1'
